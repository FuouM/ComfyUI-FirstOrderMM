...
class FSRTPosEncoder(nn.Module):
    def __init__(
        self, kp_octaves=4, kp_start_octave=-1, pix_start_octave=-1, pix_octaves=16
    ):
        super().__init__()
        self.kp_encoding = PositionalEncoding(
            num_octaves=kp_octaves, start_octave=kp_start_octave
        )
        self.pix_encoding = PositionalEncoding(
            num_octaves=pix_octaves, start_octave=pix_start_octave
        )
    ...
    def forward(self, pixels, kps=None):
        if len(pixels.shape) == 4:
            ...
            if kps is not None:
                ...
                kp_enc = kp_enc.view(batchsize, kp_enc.shape[-1], 1, 1).repeat(
                    1, 1, height, width
                )
