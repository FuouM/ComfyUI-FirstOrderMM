...
from .layers import FSRTPosEncoder, Transformer
...
class FSRTPixelPredictor(nn.Module):
    def __init__(
        ...
        self.positional_encoder = FSRTPosEncoder(
        ...
        if input_mlp:  # Input MLP added with OSRT improvements
            self.input_mlp = nn.Sequential(
                nn.Linear(self.feat_dim, 720), nn.ReLU(), nn.Linear(720, self.feat_dim)
            )
        else:
            self.input_mlp = None
        ...
        if output_mlp:
            self.output_mlp = nn.Sequential(
                nn.Linear(self.feat_dim, 128), nn.ReLU(), nn.Linear(128, out_dims)
            )
        else:
    ...
    def forward(self, z, pixels, keypoints, expression_vector=None):
        ...
        if expression_vector is not None:
            queries = torch.cat(
                [queries, expression_vector[:, None].repeat(1, queries.shape[1], 1)],
                dim=-1,
            )
...
class ImprovedFSRTDecoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.allocation_transformer = FSRTPixelPredictor(
    ...
    def forward(self, z, x, pixels, expression_vector=None):
        x = self.allocation_transformer(
            z, x, pixels, expression_vector=expression_vector
        )
