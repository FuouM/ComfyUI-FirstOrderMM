...
from .util import AntiAliasInterpolation2d, DownBlock2d, Hourglass, make_coordinate_grid
...
class KPDetector(nn.Module):
    ...
    def __init__(
        ...
        if estimate_occlusion:
            ...
            kp_occlusion.append(
                DownBlock2d(
                    block_expansion, block_expansion * 2, kernel_size=3, padding=1
                )
            )
            kp_occlusion.append(
                DownBlock2d(
                    block_expansion * 2, block_expansion * 3, kernel_size=3, padding=1
                )
            )
            kp_occlusion.append(
                DownBlock2d(
                    block_expansion * 3, block_expansion * 4, kernel_size=3, padding=1
                )
            )
            kp_occlusion.append(
                nn.Conv2d(
                    block_expansion * 4, num_kp, kernel_size=(4, 4), padding=0, stride=4
                )
            )
