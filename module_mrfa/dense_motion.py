...
class DenseMotionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.hourglass = Hourglass(
            ...
            in_features=infeatures * (num_channels + 1),
            max_features=max_features,
            num_blocks=num_blocks,
        )
        self.mask = nn.Conv2d(
            self.hourglass.out_filters, infeatures, kernel_size=(7, 7), padding=(3, 3)
        )
    ...
    def create_heatmap_representations(self, source_image, kp_driving, kp_source):
        ...
        gaussian_driving = kp2gaussian(
            kp_driving["kp"], spatial_size=spatial_size, kp_variance=self.kp_variance
        )
        gaussian_source = kp2gaussian(
            kp_source["kp"], spatial_size=spatial_size, kp_variance=self.kp_variance
        )
    ...
    def create_sparse_motions(self, source_image, kp_driving, kp_source, bg_param=None):
        ...
        coordinate_grid = identity_grid - kp_driving["kp"].view(
        ...
        driving_to_source = coordinate_grid + kp_source["kp"].view(
            bs, self.num_kp, 1, 1, 2
        )
        ...
        if bg_param is None:
            ...
            bg_grid = torch.matmul(
                bg_param.view(bs, 1, 1, 1, 3, 3), bg_grid.unsqueeze(-1)
            ).squeeze(-1)
    ...
    def forward(
        ...
        sparse_motion = self.create_sparse_motions(
            source_image, kp_driving, kp_source, bg_param=bg_param
        )
        # n * 11 * h * w * 2
...
class TPSDenseMotionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.maps = nn.Conv2d(
            hourglass_output_size, num_tps + 1, kernel_size=(7, 7), padding=(3, 3)
        )
        ...
        if multi_mask:
            ...
            occlusion = [
                nn.Conv2d(hourglass_output_size, 1, kernel_size=(7, 7), padding=(3, 3))
            ]
            self.occlusion = nn.ModuleList(occlusion)
